...
sayHelloButton.addEventListener(`mouseover`, paintItBlack);
...
sayHelloButton.addEventListener(`mouseout`, unpaint);
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/cool`)
    .then(res => {
...
function getQuery(){
    axios.get(`http://localhost:3000/query-test?iscool=true&really=true`)
    .then(res => console.log(res.data))
    .catch(error => console.log(error))
}

document.querySelector(`#query-button`).addEventListener(`click`, getQuery);
...
/*
    ...
    1: Send no queries on the URL -- what happened? The response read "You sent an empty query!" 

    2: Send more than 1 query on the URL -- what happened? The response was an object. The first property was a message with the value "You sent more than 1 query!". The second property was an object with my queries and their values set as the object properties.
*/
...
function createFood(event){
    ...
    .then(res => {
        ...
        newP.textContent = res.data[foodCount];
        document.querySelector(`body`).appendChild(newP);
        foodCount++;
    })
    foodInput.value = ``;
